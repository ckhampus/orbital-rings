...
public partial class CitizenManager : SafeNode
{
    ...
    public override void _Process(double delta)
    {
        ...
        // When found, propagate to all existing citizens whose _grid is still null
        // (starter citizens spawned during _Ready before Ring existed).
        if (_grid == null)
        {
            var ringVisual = GetTree().Root.FindChild("Ring", true, false) as RingVisual;
            if (ringVisual != null)
